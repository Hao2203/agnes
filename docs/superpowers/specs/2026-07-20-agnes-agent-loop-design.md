...
把 `agnes chat` 从"NL → 一段 DSL → 一次执行 → 结束"的**一次性规划器**改造为"NL → 多轮 (DSL → 执行 → observation 回传) → 直到 LLM 主动终止"的**多轮 agent loop**，同时保留 agnes 的核心哲学：**LLM 的输出永远是结构化 DSL，而不是 tool_use**。

具体来说，用户在 REPL 里说一句自然语言，之后：

1. LLM 输出一段 DSL（类型必须归结到 `Finish a` 或 `Observation a`，见 §2）。
2. Runtime 执行这段 DSL，产出一个 `Value`（带 `data + declared_type`）。
3. **若 declared_type 是 `Observation a`**：把 `Value.data` 通过 `a` 类型的 `show` 实现序列化成字符串，作为一条 `tool_result`-风格的 observation message 塞回 planner 的 message history，进入下一轮。
4. **若 declared_type 是 `Finish a`**：把序列化后的字符串作为最终答复展示给用户，本次 REPL turn 结束，等下一个用户输入。
5. 兜底：`MAX_TURNS = 20` 轮后强制终止。

失败（parse / check / compile / execute 任一环节报错）也作为 observation 回传给 LLM，让它自己决定重试还是换方案。
...
```
Finish a       -- runtime 用它标注 "把 a::show(data) 交给用户，终结 turn" 的 Value
...
**关键**：这两个类型**只在运行时**出现在 `Value.declared_type` 里——`finish` / `observe` 的 native_dispatch 实现负责改写。checker 层不做"根表达式的类型是 Finish/Observation"这种类型推导（agnes 不支持类型变量，见 §2.4）。checker 层只做**语法**层面的根形状检查（Rule 3，见 §2.4）。
...
checker 层的 Rule 3（见 §2.4）是**语法**规则，不是类型规则：它检查"根表达式的末尾 tool call 名字是 `finish` 还是 `observe`"，不做类型推导。这样绕过了缺失的泛型系统。

用法示例：

```agnes
;; 直接终结：把字符串字面量交给 finish
;; declared_type 从 (String) → (Finish String)
(pipe "任务完成，已翻译。" finish)

;; 执行后终结：把总结呈给用户
;; declared_type 从 (Summary) → (Finish Summary)
(pipe (tool read-file :path "README.md")
      (tool summarize)
      finish)

;; 执行后观察：让 LLM 拿到总结、决定下一步
;; declared_type 从 (Summary) → (Observation Summary)
(pipe (tool read-file :path "README.md")
      (tool summarize)
      observe)
```

### 2.4 为什么 checker Rule 3 是语法规则而非类型规则

`agnes-checker` / `agnes-registry` **不支持类型变量**：所有 tool signature 里的类型都是具体的（`PlainText`、`Summary`、`List (| PlainText Markdown)` 等）。如果按 Haskell 风格给 `finish` 一个泛型签名 `forall a. a -> Finish a`，需要引入类型变量、let-polymorphism、unification——工作量远超本设计的范围（另外一份 spec 的规模）。

替代方案：

- **静态**：checker 只保证 DSL 根表达式**语法上**以 `finish` 或 `observe` 的 tool call 收尾（Rule 3，见下）。它**不**保证根类型是 `Finish _`。
- **动态**：runtime 里 `finish` / `observe` 的 native_dispatch 实现把返回 Value 的 `declared_type` 改写为 `App{Finish, [inner]}` / `App{Observation, [inner]}`，其中 `inner` 是**运行时**上游 Value 的 declared_type。
- **Session 层分派**：Session 只看 runtime 返回的 `Value.declared_type` 的最外层 head 是 `Finish` 还是 `Observation`，不做类型推导。

**Rule 3 (Root shape，语法规则)**：作为 turn 提交给 Session 的顶层表达式（无论是 planner 产出的还是 RawDsl），其根**符号**必须是：

- `(tool finish ...)` 或 `(tool observe ...)`；或
- `(pipe ... finish)` 或 `(pipe ... observe)`（`finish` / `observe` 是 pipe 的最后一步）；或
- `(if <cond> <a> <b>)` 其中 `<a>`、`<b>` 都通过本规则；同理 `(match ...)` 每个 arm、`(let x = ... in body)` 中的 body、`(par a b)` **不允许**（并行分支多余一个 Finish/Observation 语义不明——直接拒绝）。

用户自定义 `(define f ... body)` 内部**不**受 Rule 3 约束（body 可以是任意类型）；只有作为 Session 输入的根表达式受此约束。

checker 在实现 Rule 3 时递归下降表达式的"控制流叶子"（tail 位置），检查每个 tail 是否是 `finish` / `observe` tool call。这是纯语法遍历，跟现有 Rule 1/2 的类型检查解耦。
...
run_turn(input):
  # 1. seed
  match input {
    NL(nl) => planner.begin_user_turn(nl)
    RawDsl(dsl) => 直接跳到 try_execute(dsl)，见下方 §3.1 特殊路径
  }
  for iter in 0..MAX_TURNS:
    emit IterationStart{iter}
    dsl_source = planner.plan_next()  # LLM call；输入是 messages history
    result = try_execute(dsl_source)   # parse → check(Rule 3) → compile → execute → Value
    match result {
      Ok(value) => match classify_root(&value) {
        RootKind::Finish => {
          let s = registry.show_value(&value)  // 递归剥 Finish 外层
          emit TurnResult{value: s.clone()}
          planner.record_finish(dsl_source, s)
          return Ok(value)
        }
        RootKind::Observation => {
          let inner = extract_inner_type(&value.declared_type)  // Finish/Observation 剥一层
        ...
        RootKind::Unknown => {
          // Rule 3 已在 check() 拦下，这里理论上不可达。防御性：当作 error。
          let msg = "runtime returned non-Finish/Observation value (Rule 3 escape)"
          emit ObservationEmitted{iter, text: msg.into(), is_error: true}
          planner.push_observation(dsl_source, msg.into(), true, None)
          continue
        }
      }
      Err(e) => {
...
**RawDsl 输入的特殊处理**：`/run <dsl>` slash 命令的语义保持"直接执行这段 DSL"。但根类型仍必须归结到 `Finish _` 或 `Observation _`；若是 `Observation _`，把 observation 回传后**不**继续下一轮（因为没有 NL 意图让 LLM 接力），而是把 observation 文本作为 `TurnResult` 展示给用户。**RawDsl 天然是"一次性"的**，跟 NL 走多轮 loop 语义不同。
...
impl Planner {
    ...
    /// 收到 Finish：把当前 in-flight iterations（最后一次的 observation=None）
    /// 和 Finish 一起 commit 到 history。result 是 show_value 出来的字符串。
...
| `agnes-checker` | 新增 **Rule 3 (Root shape，语法规则)**：递归下降 tail 位置，要求每个 tail 是 `finish` / `observe` tool call；`par` 不能作为 tail；`define` 内部不启用。**不做**类型推导，纯 AST 遍历。 |
...
| `agnes-session` | 大改：`SessionEvent` **先加 `#[non_exhaustive]`**（作为独立的准备提交，方便后续新增变体），然后新增 `IterationStart` / `ObservationEmitted`；`run_turn` 重构为 loop + observation XML 包装 + 截断 + MAX_TURNS + 错误回传。Session 内加 helper `classify_root(value: &Value) -> RootKind { Finish, Observation, Unknown }`（读 Value.declared_type 最外层 head）。 |
| `agnes-cli` | `StderrEventSink` 新增两个事件的渲染；`--max-turns` flag；`/history` 显示更新；现有 `examples/*.agnes` 加 `finish` 结尾。 |
...
- **`agnes-checker`**：Rule 3 对根类型不是 Finish/Observation 报错；对内部 define 不启用。
...
- **一轮直接 finish**：mock 返回 `(pipe (tool literal :value "done") finish)`；断言 `TurnResult { value: "done" }`。
...
- **执行错误自愈**：mock iter 0 返回 checker 会拒绝的 DSL，iter 1 返回正确 DSL；断言 iter=0 emit ObservationEmitted{is_error=true}，iter=1 完成。
...
- **RawDsl + Observation**：`/run (pipe summarize observe)` 应该把 observation 文本作为 TurnResult 展示，不进入下一轮。
- **RawDsl + Finish**：`/run (pipe summarize finish)` 应该 TurnResult 展示，同上。
- **RawDsl 无根类型**：`/run (pipe summarize)` 应该 checker 拒绝（Rule 3）。

### 9.3 CLI 快照/端到端

- `plan_view` 快照测试：多轮 iteration 时 stderr 输出结构（`── iteration 2 ──` 分隔、observation 行的格式）。
- Acceptance（现有 `acceptance.rs`）：`native_dispatch` 手工组装场景要迁移——之前的"跑一段 DSL 就完"现在必须以 `finish` 结尾。这是**测试迁移**，不是 API 破坏。
...
2. `SessionEvent` 新增两个变体（enum 未标 `#[non_exhaustive]` — 需要事先加，或接受编译期 fallthrough 影响所有 `match`）。
3. checker Rule 3 会拒绝任何**当前**通过测试的没有 `finish` / `observe` 根表达式的 DSL 程序——现有 `examples/*.agnes` 全部需要在末尾加 `finish`（这是"测试和示例迁移"，不是 API 破坏）。
4. Turn 结构改（`assistant_dsl / result_preview` → `iterations`）。`/history` 命令输出格式变化。
...
不是这份 spec 的一部分，只是让读者预估规模。真正的任务拆分由 `superpowers:writing-plans` 产出。粗略预计 10-14 个任务：
...
3. `agnes-builtins` 注册所有现有类型的 show impl（PlainText / Summary / TranslatedText / Markdown / PDF / Unit）。`show_value` 中内建 `List T` / `Option T` 组合规则。
4. `agnes-registry::resolve` 支持 `Finish _` / `Observation _` 作为 App head（当前 App 处理路径应该已经支持，需验证）。
5. `agnes-checker` Rule 3（语法遍历） + 测试。
6. `agnes-builtins` 注册 `finish` / `observe` tool（signature `Unknown -> Unknown`）+ native_dispatch 实现（改写 declared_type）+ 测试。
7. `agnes-llm::planner` 新接口 (`begin_user_turn` / `plan_next` / `push_observation` / `record_finish`) + 新 `Turn`/`Iteration`/`Observation` 结构 + 单元测试。
8. `agnes-llm::planner` system prompt 大改 + few-shot 更新 + 单元测试（观察 mock request messages 的 role 交替）。
9. `agnes-session::SessionEvent` 加 `#[non_exhaustive]`（**独立提交**，方便后续新增变体不惊扰 downstream）。
10. `agnes-session` 新增 `IterationStart` / `ObservationEmitted` 变体 + `classify_root` helper。
11. `agnes-session::run_turn` 重构为 loop + observation XML 包装 + 截断 + MAX_TURNS + 错误回传 + 集成测试。
12. `agnes-session` Ctrl-C 取消支持（cancellation token + `run_turn` 感知）。
13. `agnes-cli::sink_stderr` 渲染新事件；`--max-turns` flag。
14. `agnes-cli` `/history` 更新 + 迁移现有 `examples/*.agnes` 加 `finish`；`examples/chat-demo.md` 更新 + `README.md` 更新 + workspace `cargo test` + `cargo clippy --deny warnings`。

## 12. 已知遗留 / 明确不做

- **不做**：DSL 内部循环 (`loop-until` / 递归重执行)。控制流权全部交给外层 Session loop。
- **不做**：`observe` 里嵌套 `finish` 之类的复合语义——`checker` 只看根节点最外层，不做深度分析。
